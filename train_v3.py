...
def main():
    ...
    val_loader = DataLoader(
        ...
        pin_memory=True,
        persistent_workers=max(1, cfg['train']['num_workers'] // 2) > 0,
        collate_fn=collate_fn,
    )
    ...
    for epoch in range(epochs):
        ...
        for imgs, labels_list, _ in pbar:
            ...
            if not torch.isfinite(loss):
                print(f'  [WARN] skip non-finite loss: {loss.item()}')
                optimizer.zero_grad(set_to_none=True)
                continue
        ...
        if (epoch + 1) % val_interval == 0 or epoch == epochs - 1:
            ...
            val_conf_thr = cfg['train'].get('val_conf_thr', 0.01)
            val_iou_thr = cfg['train'].get('val_iou_thr', 0.5)
            val_model = ema.ema if cfg['train'].get('val_use_ema', False) else model
            ap, mean_iou, recall = evaluate(
                val_model, val_loader, device, img_size, model.strides,
                conf_thr=val_conf_thr, iou_thr=val_iou_thr
            )
