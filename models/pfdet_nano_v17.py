...
class PFDetNanoV17(nn.Module):
    ...
    def __init__(self, base_c=None, neck_c=None, num_bifpn=None, area=None,
                 ...
                 p2_ratio=0.5, head_ratio=0.75, nms_free=False):
        super().__init__()
        ...
        nc2_mid = max(16, int(nc2 * head_ratio))
        self.head_p2 = LiteHead(nc2, mid_c=nc2_mid, activation=activation)
        ...
        # ---- NMS-free: thêm head o2o (one-to-one) song song, dùng CÙNG feature ----
        if self.nms_free:
            self.head_p2_o2o = LiteHead(nc2, mid_c=nc2_mid, activation=activation)
            self.head_p3_o2o = LiteHead(neck_c, mid_c=head_c, activation=activation)
            self.head_p4_o2o = LiteHead(neck_c, mid_c=head_c, activation=activation)
            self.head_p5_o2o = LiteHead(neck_c, mid_c=head_c, activation=activation)
    ...
    def _o2m(self, feats):
        hs = [self.head_p2, self.head_p3, self.head_p4, self.head_p5]
        return [h(f) for h, f in zip(hs, feats)]

    def _o2o(self, feats):
        hs = [self.head_p2_o2o, self.head_p3_o2o, self.head_p4_o2o, self.head_p5_o2o]
        return [h(f) for h, f in zip(hs, feats)]

    def forward(self, x):
        """Mặc định: o2m outputs (giống v17 thường). NMS-free inference -> dùng o2o (ít trùng)."""
        feats = self._feats(x)
        if self.nms_free and not self.training:
            return self._o2o(feats)        # eval NMS-free: dùng head one-to-one (decode KHÔNG cần NMS)
        return self._o2m(feats)

    def forward_dual(self, x):
        """Train NMS-free: trả (o2m_list, o2o_list) để tính 2 loss."""
        feats = self._feats(x)
        return self._o2m(feats), self._o2o(feats)
