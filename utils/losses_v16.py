"""
PFDet-Nano v16 Loss Function
============================

Standalone loss file for v16.

Policy:
  - Keep the v15 training logic: TAL + STAL + ProgLoss + QFL + NWD/CIoU + ASL
  - Keep the empty-GT fix so VisDrone negative-only frames do not crash training
  - Keep normalized logging for n_pos / pos_s* to match the v14-style scale

This file keeps the v16-specific assignment and loss scheduling logic local,
but reuses shared box geometry helpers from utils/box_ops.py.
"""
...
def _tal_assign_one_scale_v16(
    ...
    for gi in range(len(gt_boxes_xywh)):
        ...
        def _compute_alignment(rows, cols):
            ...
            p_x1_t = p_cx_t - p_w_t / 2
            p_y1_t = p_cy_t - p_h_t / 2
            p_x2_t = p_cx_t + p_w_t / 2
            p_y2_t = p_cy_t + p_h_t / 2
        ...
        if is_micro and actual_k < stal_k_min:
            ...
            if len(cand_r2) > len(cand_r):
                ...
                topk_idx = (
                    alignment.topk(actual_k2)[1]
                    if actual_k2 < m2
                    else torch.arange(m2, device=device)
                )
...
class PFDetLossV16:
    ...
    def _obj_target_schedule(self, iou, epoch):
        ...
        floor = self.hard_floor * max(
            0.0, 1.0 - epoch / max(1, self.obj_warmup_epochs * 2)
        )
    ...
    def _prog_k(self, epoch):
        ...
        return (
            max(3, round(self.k_tiny * (1.0 - ratio) + ratio)),
            max(2, round(self.k_normal * (1.0 - ratio) + ratio)),
        )

    def summary(self):
        return (
            f"PFDetLossV16 (QFL + NWD/CIoU + STAL[γ={self.stal_gamma}] "
    ...
    def __call__(self, preds, labels_list, epoch=0):
            ...
            for si, (pred, stride) in enumerate(zip(preds, self.strides)):
                ...
                if g == 0:
                    qfl = quality_focal_loss(
                        obj_logits_map.view(-1),
                        obj_targets_map.view(-1),
                        self.qfl_beta,
                    )
                ...
                with torch.no_grad():
                    rows, cols, gi_list = _tal_assign_one_scale_v16(
                        raw_b,
                        gt_boxes_xywh.to(device),
                        stride,
                        h,
                        w,
                        self.img_size,
                ...
                if n_pos > 0:
                    ...
                    asl_w = (
                        (self.asl_area_ref * self.img_size ** 2 / area_px).sqrt().clamp(1.0, self.asl_max)
                    )
                    box_loss_per = box_loss_per * asl_w

                    with torch.no_grad():
                        iou_vals = bbox_iou_xyxy(pred_xyxy, gt_xyxy)
        ...
        obj_loss = total_obj_loss / batch
        box_loss = total_box_loss / max(batch, 1)
        ...
        loss_dict = {
            ...
            "box": box_loss.item(),
            "iou": mean_iou,
            "n_pos": n_pos_total / batch,
            **{
                f"pos_s{self.strides[i]}": n_pos_per_scale[i] / batch
