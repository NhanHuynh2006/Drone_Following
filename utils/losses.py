...
class SimOTAAssigner:
    ...
    def assign(self, pred_raw, gt_boxes, gt_foots, stride, img_size):
        ...
        for gi in range(n_gt):
            ...
            # Assign targets
            for k, pidx in enumerate(pos_idx):
                gj = pidx // W
                gi_x = pidx % W

                pos_mask[0, gj, gi_x] = 1.0
                # Soft objectness label = IoU (better than hard 1.0)
                cur_iou = pair_iou[topk_idx[k]].item() if len(topk_idx) > k else 1.0
                # Use a stronger positive target so confidence calibrates better for evaluation
                obj_target[0, gj, gi_x] = max(obj_target[0, gj, gi_x].item(), 1.0)

                # Box target (offsets relative to grid cell)
                tx = gt_cx * img_size / stride - gi_x.float()
                ty = gt_cy * img_size / stride - gj.float()
                tw = torch.log(torch.clamp(gt_w * img_size / stride, min=1e-6))
                th = torch.log(torch.clamp(gt_h * img_size / stride, min=1e-6))
    ...
    def forward(self, predictions, targets_list):
        ...
        for si, (pred, stride) in enumerate(zip(predictions, self.strides)):
            ...
            for b in range(B):
                ...
                raw_npos = pmask.sum()
                total_pos += raw_npos.item()

                if raw_npos.item() > 0:
                    pred_sig = pred[b]
                    pm = pmask[0] > 0.5

                    pos_ij = torch.nonzero(pm, as_tuple=False)
                    if pos_ij.numel() > 0:
                        ...
                        ciou_loss = bbox_ciou(pred_xyxy, tgt_xyxy)
                        ciou_loss = torch.nan_to_num(ciou_loss, nan=0.0, posinf=0.0, neginf=0.0).mean()
                        ...
                        foot_loss_x = F.smooth_l1_loss(pred_fx, tgt_fx, beta=0.02, reduction='mean')
                        foot_loss_y = F.smooth_l1_loss(pred_fy, tgt_fy, beta=0.02, reduction='mean')
                        foot_loss = torch.nan_to_num(foot_loss_x + foot_loss_y, nan=0.0, posinf=0.0, neginf=0.0)
